...
//	i2c->CR2 = I2C_CR2_START | I2C_CR2_AUTOEND | (2 << 16) | (saddr << 1);
	i2c->CR2 = I2C_CR2_START | (saddr << 1);
...
void I2C_MasterByteWrite(I2C_TypeDef *i2c, uint8_t saddr, uint8_t maddr,
		uint8_t data) {
	while (i2c->ISR & I2C_ISR_BUSY)
		; /* wait until bus not busy */
...
void I2C_MasterByteRead(I2C_TypeDef *i2c, uint8_t saddr, uint8_t maddr,
		uint8_t *data) {
	while (i2c->ISR & I2C_ISR_BUSY)
		; /* wait until bus not busy */
...
void I2C_MasterMultiWrite(I2C_TypeDef *i2c, uint8_t saddr, uint8_t maddr,
		uint32_t n, uint8_t *data) {
	int i;
...
void I2C_MasterMultiRead(I2C_TypeDef *i2c, uint8_t saddr, uint8_t maddr,
		uint32_t n, uint8_t *data) {
	while (i2c->ISR & I2C_ISR_BUSY)
		; /* wait until bus not busy */
